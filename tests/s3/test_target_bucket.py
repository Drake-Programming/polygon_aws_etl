...
class TestTargetBucketConnector(TestBaseBucketConnector):
    ...
    test_csv_format = "csv"
    test_df = pd.DataFrame([["A", "B"], ["C", "D"]], columns=["col1", "col2"])
    # for test parquet io
    ...
    def test_write_to_s3_csv(self):
        ...
        data = (
            self.bucket.Object(key=self.test_csv_key)
            .get()
            .get("Body")
            .read()
            .decode("utf-8")
    ...
    def test_write_to_s3_parquet(self):
        """
        test write_to_s3 works for parquet
        """
        ...
        data = self.bucket.Object(key=self.test_parquet_key).get().get("Body").read()
        out_buffer = BytesIO(data)
        df_result = pd.read_parquet(out_buffer)
        self.assertTrue(df_result.equals(df_expected))

    def test_write_to_s3_wrong_format(self):
        """
        test write_to_s3 generates expected error and logs when given unsupported file formats
        """
    ...
    def test_read_meta_file(self):
        ...
        csv_content = (
            f"{self.date_col},{self.timestamp_col}\n{self.test_date},{test_timestamp}"
        )
